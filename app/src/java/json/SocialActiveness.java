    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            for (HashMap<String, Breakdown> appDetails : appList) {
                sortMap.put(appDetails.get("app-name").getMessage(), Integer.parseInt(appDetails.get("percent").getMessage()));
            }

            Iterator<String> iter = sortMap.keySet().iterator();
            while (iter.hasNext()) {
                String name = iter.next();
                eachApp.addProperty("app-name", name);
                eachApp.addProperty("percent", sortMap.get(name));
                arr.add(eachApp);
